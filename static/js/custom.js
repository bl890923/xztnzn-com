...
jQuery(window).load(function(){
	...
	if ($featured_content.length) {
		var $featured_slides = $featured_content.find('.slide').show(),
			slides_pos = [],
			slides_zindex = [],
			active_slide_width = 600,//澶у剧
			small_slide_width = 355,//灏剧
			slide_margin = 16,
		...
		$featured_slides.each(function(index, domEle){
			...
			if ( index === 0 ) {
				top_slide_pos = 0;
				slide_opacity = 1;
				left_slide_pos = 0;//澶у剧宸杈圭璺绂
			}
			if ( index === 1 ) left_slide_pos = 500;//璋存翠釜剧宸抽搴
		...
		function rotate_slide(direction){
			...
			$featured_slides.each(function(index, domEle){
				...
				if ( next_slide_num != 0 ) {
					$this_slide.find('img').stop(true, true).animate({'width':'355px','height':'266px'},600,featured_animation);//淇瑰剧澶у
				}
				else { 
					$this_slide.find('img').stop(true, true).animate({'width':'600px','height':'450px'},600,featured_animation,function(){//淇瑰ぇ剧澶у
